...
impl VaultSeal for FakeSeal {
    fn seal(&self, namespace: [u8; 16], epoch: u64, plain: &[u8]) -> Result<Vec<u8>, PlatformError> {
        let mut bytes = Vec::new();
        bytes.extend_from_slice(&namespace);
        bytes.extend_from_slice(&epoch.to_be_bytes());
        bytes.extend(plain.iter().map(|b| b ^ 0xa5));
        let checksum = bytes.iter().fold(0u64, |n, &b| n.wrapping_mul(131).wrapping_add(u64::from(b)));
        bytes.extend_from_slice(&checksum.to_be_bytes());
        Ok(bytes)
    }
    fn open(&self, namespace: [u8; 16], epoch: u64, sealed: &[u8]) -> Result<Vec<u8>, PlatformError> {
        ...
        let checksum = data.iter().fold(0u64, |n, &b| n.wrapping_mul(131).wrapping_add(u64::from(b)));
        if digest != checksum.to_be_bytes() { return Err(PlatformError::Corrupt); }
        Ok(data[24..].iter().map(|b| b ^ 0xa5).collect())
    }
}

type Grants = Rc<RefCell<alloc::collections::BTreeSet<(AppId, VaultOwnerId, Option<ObjectId>, VaultAction)>>>;
struct Policy(Grants);
impl VaultAuthorizer for Policy {
    fn permits(&self, caller: AppId, owner: VaultOwnerId, id: Option<ObjectId>, action: VaultAction) -> bool {
...
fn setup() -> (TestVault, InMemoryVaultStateStore, Grants) {
    ...
    grants.borrow_mut().insert((ALICE, OWNER, None, VaultAction::Create));
    let vault = TestVault::initialize(store, FakeSeal, DeterministicRandom::new(1), Policy(grants.clone())).unwrap_or_else(|_| panic!("provision failed"));
    (vault, fork, grants)
}
fn permit(grants: &Grants, caller: AppId, owner: VaultOwnerId, id: Option<ObjectId>, action: VaultAction) {
...
fn reopen(store: InMemoryVaultStateStore, grants: Grants) -> Result<TestVault, VaultError> {
    TestVault::open(store, FakeSeal, DeterministicRandom::new(77), Policy(grants))
}

#[test]
fn explicit_provision_and_authority_before_lookup() {
    let store = InMemoryVaultStateStore::new(NS);
    assert!(matches!(reopen(store, Rc::new(RefCell::new(Default::default()))), Err(VaultError::Unprovisioned)));
    let (mut vault, fork, grants) = setup();
    let id = vault.create(ALICE, OWNER, 1, b"private title", b"secret content").unwrap();
    ...
    assert_eq!(vault.read(ALICE, OWNER, ObjectId::from_bytes([99; 16])).err(), Some(VaultError::Denied));
    ...
    assert_eq!(vault.create(BOB, OWNER, 1, b"", b""), Err(VaultError::Denied));
    drop(vault);
    assert!(matches!(TestVault::initialize(fork, FakeSeal, DeterministicRandom::new(1), Policy(grants)), Err(VaultError::AlreadyProvisioned)));
}
...
fn replace_delete_restart_and_owner_mismatch() {
    ...
    assert_eq!(vault.replace(ALICE, OTHER, id, b"m1", b"v1"), Err(VaultError::Denied));
    let first_ref = *vault.read(ALICE, OWNER, id).unwrap().content_ref();
    assert_eq!(vault.replace(ALICE, OWNER, id, b"m1", b"v1"), Ok(2));
    assert_ne!(*vault.read(ALICE, OWNER, id).unwrap().content_ref(), first_ref);
    drop(vault);
    ...
    drop(reopened);
    assert_eq!(reopen(fork, grants).unwrap().read(ALICE, OWNER, id).err(), Some(VaultError::Denied));
}
...
fn corruption_rollback_and_context_are_rejected() {
    ...
    let id = vault.create(ALICE, OWNER, 2, b"private metadata", b"private content").unwrap();
    let current = fork.snapshot_for_test().unwrap();
    assert_ne!(old.0, current.0);
    drop(vault);
    fork.replace_snapshot_for_test(Some(old));
    assert!(matches!(reopen(fork.fork_for_test(), grants.clone()), Err(VaultError::Storage(PlatformError::Corrupt))));
    fork.replace_snapshot_for_test(Some(current.clone()));
    let mut tampered = current.clone();
    tampered.1[32] ^= 1;
    fork.replace_snapshot_for_test(Some(tampered));
    assert!(matches!(reopen(fork.fork_for_test(), grants.clone()), Err(VaultError::Corrupt)));
    fork.replace_snapshot_for_test(Some(current.clone()));
    ...
    plaintext[8] = 0xff;
    fork.replace_snapshot_for_test(Some((current.0, FakeSeal.seal(NS, current.0, &plaintext).unwrap())));
    assert!(matches!(reopen(fork.fork_for_test(), grants.clone()), Err(VaultError::Corrupt)));
    plaintext[8] = 0;
    plaintext[70] = 0xff;
    fork.replace_snapshot_for_test(Some((current.0, FakeSeal.seal(NS, current.0, &plaintext).unwrap())));
...
fn partial_commit_ambiguous_commit_and_lease_fence() {
    let (mut vault, fork, grants) = setup();
    assert!(matches!(reopen(fork.fork_for_test(), grants.clone()), Err(VaultError::Storage(PlatformError::Unavailable))));
    vault.store.fail_before_commit();
    assert_eq!(vault.create(ALICE, OWNER, 1, b"a", b"b"), Err(VaultError::Storage(PlatformError::Unavailable)));
    ...
    assert_eq!(reopened.create(ALICE, OWNER, 1, b"a", b"b"), Err(VaultError::Storage(PlatformError::Unavailable)));
    drop(reopened);
...
fn bounds_entropy_and_parser_mutations() {
    let (mut vault, fork, grants) = setup();
    assert_eq!(vault.create(ALICE, OWNER, 0, b"", b""), Err(VaultError::InvalidInput));
    assert_eq!(vault.create(ALICE, OWNER, 1, &alloc::vec![0; MAX_METADATA + 1], b""), Err(VaultError::InvalidInput));
    assert_eq!(vault.create(ALICE, OWNER, 1, b"", &alloc::vec![0; MAX_CONTENT + 1]), Err(VaultError::InvalidInput));
    ...
    for value in 1..=(MAX_OBJECTS + 1) {
        ...
        too_many.insert(id, VaultObject {
            id, owner: OWNER, kind: 1, content_ref: [(value + 100) as u8; 16],
            revision: 1, metadata: Vec::new(), content: Vec::new(),
        });
    }
