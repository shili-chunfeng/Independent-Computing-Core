...
use icc_platform_api::{
    CapabilityClock, CapabilityStateStore, Clock, KeyStateStore, ObjectStore, SecretStore,
    SecureRandom, SecurityStateStore, VaultStateStore,
};
...
/// A shared clock with a reboot-stable epoch for authority state-machine tests.
/// Real monotonic-clock persistence/freshness is a platform responsibility.
#[derive(Clone)]
pub struct FakeCapabilityClock(Rc<RefCell<u64>>);

impl FakeCapabilityClock {
    pub fn new(now: u64) -> Self {
        Self(Rc::new(RefCell::new(now)))
    }

    pub fn set_for_test(&self, now: u64) {
        *self.0.borrow_mut() = now;
    }
}

impl CapabilityClock for FakeCapabilityClock {
    fn trusted_time_ms(&self) -> Result<MonotonicMs, PlatformError> {
        Ok(MonotonicMs::new(*self.0.borrow()))
    }
}
...
/// Separate capability security-state namespace; reuses only the simulated
/// lease/epoch/fault state machine, never a Vault or KeyStore instance.
pub struct InMemoryCapabilityStateStore(InMemoryKeyStateStore);

impl InMemoryCapabilityStateStore {
    pub fn new(namespace: [u8; 16]) -> Self {
        Self(InMemoryKeyStateStore::new(namespace))
    }
    pub fn fork_for_test(&self) -> Self {
...
impl CapabilityStateStore for InMemoryCapabilityStateStore {
    fn acquire_exclusive(&mut self) -> Result<(), PlatformError> {
        self.0.acquire_exclusive()
    }
    fn release_exclusive(&mut self) {
